...
#include "RandomLightBuoyPlugin.hh"

using namespace gz;
using namespace vrx;

/// \brief Private RandomLightBuoyPlugin data class.
class RandomLightBuoyPlugin::Implementation
{
  /// \def Pattern_t
...
std::map<std::string, gz::msgs::Color>
  RandomLightBuoyPlugin::Implementation::kColors =
  {
...
//////////////////////////////////////////////////
msgs::Color RandomLightBuoyPlugin::Implementation::CreateColor(const double _r,
...
/////////////////////////////////////////////////
bool RandomLightBuoyPlugin::Implementation::ParseSDF(sdf::ElementPtr _sdf)
{
...
//////////////////////////////////////////////////
void RandomLightBuoyPlugin::Implementation::Update()
{
  if (!this->scene)
  ...
  if (this->state < 3)  // First 3 states: random colors
  {
    // Available colors for random selection
    std::vector<std::string> availableColors = {"red", "green", "blue", "yellow"};
    
    // Random selection
    int randomIndex = rand() % availableColors.size();
    selectedColor = availableColors[randomIndex];
    
    gzmsg << "Buoy selecting random color: " << selectedColor << " (state " << (int)this->state << ")" << std::endl;
  }
  else  // 4th state: off
  {
    selectedColor = "off";
    gzmsg << "Buoy off (state " << (int)this->state << ")" << std::endl;
  }

  auto color = this->kColors[selectedColor];
...
/////////////////////////////////////////////////
RandomLightBuoyPlugin::RandomLightBuoyPlugin()
  : dataPtr(utils::MakeUniqueImpl<Implementation>())
{
}

//////////////////////////////////////////////////
void RandomLightBuoyPlugin::Configure(const sim::Entity &_entity,
    ...
    sim::EventManager &_eventMgr)
{
  ...
  srand(time(NULL) + static_cast<unsigned int>(_entity));
  ...
  this->dataPtr->connection =
    _eventMgr.Connect<sim::events::SceneUpdate>(
      std::bind(&RandomLightBuoyPlugin::Implementation::Update, this->dataPtr.get()));
}

//////////////////////////////////////////////////
void RandomLightBuoyPlugin::PreUpdate(const sim::UpdateInfo &_info,
  sim::EntityComponentManager &_ecm)
{
...
GZ_ADD_PLUGIN(RandomLightBuoyPlugin,
              sim::System,
              RandomLightBuoyPlugin::ISystemConfigure,
              RandomLightBuoyPlugin::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(vrx::RandomLightBuoyPlugin,
                    "vrx::RandomLightBuoyPlugin")
